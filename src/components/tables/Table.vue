...
<script>
  ...
  export default {
    ...
    created () {
      var self = this
      var dwpApiUrl = 'https://n1isk6omo4.execute-api.us-west-2.amazonaws.com/v1/dwp'
      // Call to dryad job output
      axios.get(dwpApiUrl).then((response) => {
        console.log('Success in loading dryad job output')
        console.log(response)
        self.jobOutput = response.data
      }).catch(() => {
        console.error('Problem loading dryad job output')
      })
    },
    data () {
      return {
